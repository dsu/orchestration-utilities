...
class MakePackage(Operation):
    '''Prepares files to update an application. It selects files that are no older than 'hours_old_max' hours. It can create archive that will be uploaded to the server.
     It can check if Java .class files have specific java version.
    '''
                    ...
                    if file_mod_dt > self.newerThan:
                        ...
                        if self.jversion and pathname.endswith(('.class')):
                            ...
                            if ver != self.jversion:
                                raise Exception(
                                    ' {} is not valid {} class file ({})!'.format(pathname, self.jversion, ver))
